...
public class ReportCommand extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e){
        if (e.getMessage().getContentRaw().toLowerCase().startsWith(Getter.PREFIX + "report") && e.getMember().hasPermission(Permission.ADMINISTRATOR)){
                ...
                if (args[0].equalsIgnoreCase(Getter.PREFIX + "report")){
                    if (e.getJDA().retrieveUserById(args[1]).complete() != null){
                        ...
                        try {
                            ...
                            EmbedBuilder eb = Getter.getReport(u, report);
                            eb.addField("Reporter", "Reporter: " + e.getAuthor().getAsTag() + "\n" +
                                    "ID: " + e.getAuthor().getId() + "\n" +
                                    "Guild: " + e.getGuild().getName(), false);
                            Getter.AUTHOR.openPrivateChannel().complete().sendMessage(eb.build()).queue(message -> {
                            ...
                            for(String ids : Getter.mods){
                                ...
                                User mod = e.getJDA().retrieveUserById(ids).complete();
                                mod.openPrivateChannel().complete().sendMessage(eb.build()).queue(message -> {
                                    message.addReaction("\u2705").queue();
                                    message.addReaction("\u274C").queue();
                                });
                            }
            ...
            } else if (Getter.mods.contains(e.getAuthor().getId()) && args.length == 3){
                if (args[0].equalsIgnoreCase(Getter.PREFIX + "report") && args[1].equalsIgnoreCase("get")){
                    if (main.jda.retrieveUserById(args[2]).complete() != null){
                        if (Getter.reports.get(args[2]) != null){
                            ...
                            e.getChannel().sendMessage(Getter.getReport(main.jda.retrieveUserById(args[2]).complete(), report).build()).queue(message -> {
                                message.addReaction("\u2705").queue();
                                message.addReaction("\u274C").queue();
                            });
                        }
            ...
            } else if (Getter.mods.contains(e.getAuthor().getId()) && args.length == 2){
